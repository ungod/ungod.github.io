...
复现工程：
 ...
 [ShaderBug.rar](/assets/postasset/download/ShaderBug.rar) ：场景中有问题的模型和材质是石头，此时阴影距离为0，MainLightShadowCasterPass不会执行，_MainLightShadowmapTexture没有赋值

 [ShaderHuaweiBug.rar](/assets/postasset/download/ShaderHuaweiBug.rar) ：最小复现场景，只有一个Shader，Shader在FS只调用了SampleCmpLevelZero，阴影距离为0，项目配置为线性空间，API为OpenGLES3，其他都是默认配置。
